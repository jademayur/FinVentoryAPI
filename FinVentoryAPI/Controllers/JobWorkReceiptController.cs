...
namespace FinVentoryAPI.Controllers
{
    ...
    public class JobWorkReceiptController : ControllerBase
    {
        ...
        public JobWorkReceiptController(IJobWorkReceiptService service) { _service = service; }
        ...
        [HttpPost] public async Task<IActionResult> Create([FromBody] CreateJobWorkReceiptMainDto dto) { try { return Ok(await _service.CreateAsync(dto)); } catch (Exception ex) { return BadRequest(new { message = ex.Message }); } }
        [HttpPut("{id}")] public async Task<IActionResult> Update(int id, [FromBody] UpdateJobWorkReceiptMainDto dto) { try { var r = await _service.UpdateAsync(id, dto); if (r == null) return NotFound(new { message = "Not found or not Draft." }); return Ok(r); } catch (Exception ex) { return BadRequest(new { message = ex.Message }); } }
